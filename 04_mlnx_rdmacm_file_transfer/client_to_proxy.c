...
void * poll_cq(void *ctx)
{
    ...
    while (1) 
    {
        ...
        {
            // fprintf(stderr,"poll_cq ibv_get_cq_event success\n");
...
void event_loop(struct rdma_event_channel *ec)
{
    ...
    while (rdma_get_cm_event(ec, &event) == 0) 
    {
        ...
        {
            printf("rdma_get_cm_event() == RDMA_CM_EVENT_ADDR_RESOLVED\n");
        ...
        {
            printf("rdma_get_cm_event() == RDMA_CM_EVENT_ROUTE_RESOLVED\n");
        ...
        {
            printf("rdma_get_cm_event() == RDMA_CM_EVENT_ESTABLISHED\n");
            post_receive(event_copy.id);
            send_next_chunk(event_copy.id);
...
static void write_remote(struct rdma_cm_id *id, uint32_t len)
{
    ...
    sr.imm_data = htonl(len);

    printf("start send uuid: %s\n",ctx->buffer );

    /* there is a Receive Request in the responder side, so we won't get any into RNR flow */
    rc = ibv_post_send(id->qp, &sr, &bad_wr);
    if (rc) 
    {
...
static void on_pre_conn(struct rdma_cm_id *id)
{
    ...
    ctx->recv_buffer_mr = ibv_reg_mr(rc_get_pd(), ctx->recv_buffer, BUFFER_SIZE, IBV_ACCESS_LOCAL_WRITE);
    if (NULL == ctx->recv_buffer_mr)
    {
        rc_die("ctx->buffer_mr ibv_reg_mr() error\n");
    }
...
static void on_completion(struct ibv_wc *wc)
{
    ...
    {
        // printf("ibv_post_recv() poll_cq() success\n");
        sleep(3);
    ...
    {
        // printf("ibv_post_send() poll_cq() success  %s\n",ctx->recv_buffer);

    }
    else
    {
        // printf("on_completion other event wc->opcode : %d\n",wc->opcode);
    }
}
